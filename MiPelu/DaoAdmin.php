...
class DaoAdmin extends DB 
{
   ...
   public function listar()       //Lista el contenido de la tabla
   {
     ...
     foreach($this->filas as $fila)
     {
        ...
        $ad->__set("nombreAdmin", $fila['nombreAdmin']);
        $ad->__set("contraseña", $fila['contraseña']);
...
//Necesitamos incluir la libreria y la clase entidad asociada al DAO
    ...
    public function obtener($user, $pass) { // Obtiene el elemento por usuario y contraseña
        $consulta = "SELECT * FROM administradores WHERE nombreAdmin = :user AND contraseña = :pass";
        ...
        if (count($this->filas) == 1) { // Verifica si se encontró un resultado
            ...
            $admin->__set("nombreAdmin", $fila['nombreAdmin']);
        } else {
            echo "<b>El usuario y la contraseña no coinciden.</b>"; // Mensaje de error si no se encuentra coincidencia
        }
